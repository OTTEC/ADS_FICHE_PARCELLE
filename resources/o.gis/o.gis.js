...
// ol-ext geolocate gps
	...
	geoloc.on('position', function(e) {
	  if (e.coordinate) here.show(e.coordinate, "Vous êtes<br/>ici");
	  else here.hide();
	});	
...
// Popup WMS-WFS Query
	var querywmswfs = new ol.control.Toggle({
		...
		title: 'Interroger une couche WMS-WFS',
...
// measurement
	...
	var measurementOption = [
	  ...
	  { value: "Circle", description: "Rayon" }
	];
	// Aggiungere le opzioni alla select
	 ...
	 * @type {string}
	 */
	var continueCircleMsg = "Cliquez pour valider la forme";
...
// ol-ext editbar
	...
	editBar.getInteraction('DrawPoint').on('change:active', function(e){
	  tooltip.setInfo(e.oldValue ? '' : 'Cliquez pour placer un point');
	});
	
	editBar.getInteraction('DrawLine').on(['change:active','drawend'], function(e){
	  tooltip.setFeature();
	  tooltip.setInfo(e.oldValue ? '' : 'Cliquez pour commencer à dessiner');
	});
	editBar.getInteraction('DrawLine').on('drawstart', function(e){
	  tooltip.setFeature(e.feature);
	  tooltip.setInfo('Cliquez pour commencer à dessiner');
	});
...
// ol-ext scale control
	...
	// Set info
	scalaelement.innerHTML = "Échelle"
...
// ol-ext print dialog control
	...
	// Set button title
	printbuttontitle.attr('title', 'Imprimer');
